...
## Step 2.5: 変更対象ファイルの健全性チェック（Pre-flight）

実装計画で**変更予定のファイル**を事前に読み、既存の問題を洗い出す。
新機能を「壊れた土台の上」に積まないための予防措置。

> **起源**: CROSS-AI 分析（2026-03-27）で、`dashboard.js` の `handleSend` に `state.isTyping` ガードが欠けている既存バグを見逃したことが発端。実装後の ultra-review では発見が遅すぎるため、実装前にチェックするステップを追加。

### 防御的プログラミングチェックリスト

| # | チェック項目 | 確認内容 |
|---|------------|---------|
| 1 | **二重実行ガード** | async 関数にボタン連打/二重呼び出し防止があるか？（`isLoading` / `isTyping` 等のフラグ） |
| 2 | **リソースクリーンアップ** | `setInterval` / `addEventListener` / `onSnapshot` に対応する解除処理があるか？ |
| 3 | **エラーハンドリング** | `try-catch` が適切か？ catch 内でユーザーに通知しているか？サイレント失敗はないか？ |
| 4 | **未使用状態フラグ** | 状態フラグ（`isLoading`, `isTyping` 等）が定義されているのに **使われていない** 箇所はないか？ |
| 5 | **データ整合性** | 配列/オブジェクトの追加・削除時に、関連する他のデータ構造も更新されているか？ |

### 対応ルール

| 発見した問題の深刻度 | 対応 |
|-------------------|------|
| 🔴 **中程度以上**（二重実行ガード欠如、リソースリーク等） | 計画に「**Task 0: 既存バグ修正**」を追加して先に修正 |
| 🟡 **軽微**（命名改善、コメント不足等） | TODO コメントを残して続行 |
