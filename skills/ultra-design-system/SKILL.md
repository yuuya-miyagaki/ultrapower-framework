...
1案だけ出すのではなく、**3つのバリアント**を同時に提案してユーザーに選ばせる。

```text
プロダクトコンテキスト: [理解内容] に基づく3案:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CONSERVATIVE（安全策）— カテゴリのベースライン
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
美的方向性:    [方向] — [このカテゴリで標準的な理由]
カラー:        [Primary hex] + [Neutral hex]
タイポグラフィ: [安定したフォントの組み合わせ]
リスク:        低（ユーザー期待に沿う。記憶には残りにくい）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
BALANCED（推奨）— 1つの差別化 + ベースライン
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
美的方向性:    [方向] — [根拠]
カラー:        [Primary hex] + [Accent hex]
タイポグラフィ: [特徴的なDisplayフォント + 安定したBodyフォント]
差別化ポイント: [1つの記憶に残る選択と理由]
リスク:        中（ほとんどのユーザーに好意的に受け取られる）
...
カラー:        [予想外のパレット]
タイポグラフィ: [個性的な組み合わせ]
差別化ポイント: [複数の意図的な差別化とトレードオフ]
リスク:        高（賛否が分かれる。成功すれば圧倒的に記憶に残る）
```

「どの案が刺さりますか？一部だけ入れ替えも可能です。」

### ビジュアルモックアップ生成（オプション）

ユーザーが視覚的な確認を希望する場合、`generate_image` で各案のコンセプトイメージを生成:

```yaml
generate_image:
  prompt: |
    UI design concept for [プロダクト名], [美的方向性] aesthetic.
    Color palette: [Primary hex], [Secondary hex].
    Font style: [Displayフォント] for headings, clean typography.
    Show a [ダッシュボード/ランディングページ等] layout.
    Professional web application design, high quality mockup.
  aspect_ratio: "16:9"
```

1案あたり1枚まで。全3案を生成する前にユーザーに確認する。
...
選択した案をベースに、特定セクションを深掘り:
- **案のブレンド:** 「BALANCEDのカラーにBOLDのタイポグラフィを組み合わせたい」等の混合にも対応
