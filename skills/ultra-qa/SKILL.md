...
## Step 0.5: Context7 テストベストプラクティス参照

> テスト実行前に、使用しているテストフレームワーク・ライブラリの最新ベストプラクティスを確認。
> 既知の落とし穴（非同期テストの書き方、モックの正しいリセット方法等）を事前に把握する。

```yaml
# 1. テストフレームワークを特定
run_command: cat package.json | grep -E "(jest|vitest|mocha|playwright|cypress|testing-library)" | head -5

# 2. テストフレームワークの最新ドキュメントを取得
mcp_context7_resolve-library-id:
  libraryName: "[検出したテストフレームワーク]"
  query: "testing best practices common pitfalls async testing"

mcp_context7_query-docs:
  libraryId: "[取得したID]"
  query: "common test failures mock cleanup async patterns"
```

**参照ルール:**

- テストフレームワーク1つのみ参照（速度優先）
- 前回のQAで失敗したパターンがある場合、その原因に関連するドキュメントを優先参照
- 取得した知見は Step 5 の検証チェックリストに反映
...
### ⚠️ E2E テスト環境分離（Firebase / Supabase プロジェクト）

> **起源**: Phase 3b（2026-03-27）で E2E テスト中に本番 Firestore のプリセットデータを誤削除。テスト環境と本番環境を分離していなかったことが原因。

| DB バックエンド | 推奨テスト環境 | 設定方法 |
|----------------|---------------|---------|
| **Firebase** | Firebase エミュレータ | `firebase emulators:start --only firestore,auth` |
| **Supabase** | ローカル Supabase | `supabase start` |
| **どちらも不可** | テスト専用ワールド/テナント | テストデータに `[TEST]` プレフィックスを付与 |

**ルール**:
- 🔴 **削除・更新を伴う E2E テストは、本番データに対して絶対に実行しない**
- 🟢 読み取り専用テスト（表示確認等）は本番でも可
- テストデータの作成→検証→クリーンアップを1サイクルで完結させる
