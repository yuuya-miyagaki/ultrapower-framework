...
**観察先行原則（Observe Before Guessing）**

デバッグ時はコードを読む前にまず動かす。推測でコードを修正するな。ブラウザを開け、コンソールを見ろ、APIを叩け。10分の観察が数時間の推測より速い。

> **教訓**: マルチエージェントAIの会話品質改善で、コード分析とプロンプト改善に数時間を費やした。しかし根本原因は「APIキーのクォータがゼロだった」こと。最初にブラウザのコンソールを見ていれば10分で判明していた。
...
> **重要**: アーティファクトはフレームワーク内ではなく、**対象プロジェクトの `docs/` ディレクトリ**に出力すること。
> フレームワークはプロジェクト非依存の汎用ツールであり、特定プロジェクトの成果物を含めてはならない。
...
各スキル実行時に `mkdir -p` で自動作成。`{project-root}` はフレームワークではなく対象プロジェクトのルートディレクトリ。
